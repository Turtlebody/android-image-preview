...
//        }
            ...
            mActivity.get()?.let {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    ...
                    it.window.navigationBarColor =
                        ContextCompat.getColor(it, R.color.md_black_1000_75)
        ...
        private fun setOriginalState() {
            mActivity.get()?.let { fragment ->
                if (fragment is AppCompatActivity) {
                    mIsActionBarShowing?.let {
                        if (it) {
                            fragment.supportActionBar?.show()
                        }
                    }
                }

                fragment.window.clearFlags(WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS)

                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    mNavigationalBarColor?.let {
                        fragment.window.navigationBarColor = it
                    }

                    //status
                    mStatusBarColor?.let {
                        fragment.window.statusBarColor = it
                    }

                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        mOriginalFlag?.let {
                            fragment.window.decorView.systemUiVisibility = it
                        }
                        mStatusBarColor?.let {
                            fragment.window.statusBarColor = it
                        }
...
//            }
    ...
    class PreviewFragment : FragmentBase() {
        ...
        override fun onCreateView(
            ...
            arguments?.let {
                mPreviewConfig =
                    it.getSerializable(ImagePreviewConfig.ARG_BUNDLE) as ImagePreviewConfig
            }

            return inflater.inflate(R.layout.tb_image_preview_fragment_preview, container, false)
                .apply {
